...
func (f *HumanPrintFlags) AddFlags(c *cobra.Command) {
	...
	// TODO: How should PrintHandlers be set? They cannot be set here as that would cause an import cycle
	// unless we move `pkg/printers/internalversion/printers.go` to `pkg/printers/handlers.go` or something along those lines.
}

// NewHumanPrintFlags returns flags associated with
// human-readable printing, with default values set.
func NewHumanPrintFlags(noHeaders, withNamespace, absoluteTimestamps bool) *HumanPrintFlags {
	showLabels := false
	sortBy := ""
	showKind := false
	columnLabels := []string{}
